...
export default class AdminManageUsers extends Component {
  ...
  updateSexe(event) {
    ...
    if (event.target.name === 'usexe_m') {
      ...
      const copy = Object.assign({}, this.state.selectedUser.addresses[0]);
      copy.sexe_m = event.target.value !== '0';
      request({
      ...
      const copy = Object.assign({}, this.state.selectedUser.addresses[1]);
      copy.sexe_m = event.target.value !== '0';
      request({
                ...
                <table className="table table-sm">
                  <tbody>
                    {this.state.filteredUsers.map(user => (
                      <tr key={user.id}>
                        <td>
                          <span className="badge badge-info">{this.renderType(user.user_type)}</span>
                          {' '}
                          {(user.company_name) ? user.company_name : `${user.name} ${user.firstname}`}
                        </td>
                        <td><button className="btn btn-sm btn-link" onClick={this.selectUser.bind(this, user)}><FontAwesome name="pencil" /></button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                      ...
                      <div className="col-lg-6 col-md-12 my-2">
                        <div className="card">
                          <div className="card-block">
                            ...
                            <div>
                              <Address data={this.state.billing_address} company={this.state.selectedUser.user_type === 2} />
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="col-lg-6 col-md-12 my-2">
                        <div className="card">
                          <div className="card-block">
                            ...
                            <div>
                              <Address data={this.state.delivery_address} company={this.state.selectedUser.user_type === 2} />
                            </div>
                          </div>
                        </div>
                      </div>
...
:
                                    {' '}
                                    {this.getEmailType(elem.type_email)}
                                  </td>
                                  <td>{moment(elem.time).format('DD/MM/YY HH:mm')}</td>
                                  <td>{this.getTag(elem.state)}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
