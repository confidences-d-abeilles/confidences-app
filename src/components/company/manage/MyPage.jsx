...
export default withNotification(class CompanyManageMyPage extends Component {
  ...
  replaceNamespace(e) {
    ...
    const TabSpec = {
      à: 'a', á: 'a', â: 'a', ã: 'a', ä: 'a', å: 'a', ò: 'o', ó: 'o', ô: 'o', õ: 'o', ö: 'o', ø: 'o', è: 'e', é: 'e', ê: 'e', ë: 'e', ç: 'c', ì: 'i', í: 'i', î: 'i', ï: 'i', ù: 'u', ú: 'u', û: 'u', ü: 'u', ÿ: 'y', ñ: 'n', _: ' ',
    };
    const reg = /[àáäâèéêëçìíîïòóôõöøùúûüÿñ_]/gi;
    this.setState({
        ...
        {(this.state.user)
          ? (
            <form>
              ...
              <Input type="text" placeholder="Nom de l url" name="fakeNamespace" value={this.state.fakeNamespace} onChange={this.replaceNamespace.bind(this)} />
              <br />
              <label>{`https://parrainagederuches.fr/parrains/${this.state.namespace}`}</label>
              <hr />
              <div className="form-group">
                <label>
Photo de couverture de votre page
                  {(this.state.cover) ? <a href={`${process.env.REACT_APP_CONTENT_DOMAIN}/${this.state.cover}`} target="_blank" rel="noopener noreferrer">Visualiser l'image actuelle</a> : null}
                </label>
...
id="cover"
onChange={() => { this.setState({ newCover: document.getElementById('cover').files[0].name }); }}
style={{
                  ...
                  />
							Glissez une
                  {' '}
                  {(this.state.cover) ? 'nouvelle' : null}
                  {' '}
image ici ou cliquez pour en sélectionner une parmi vos fichiers
                  <br />
							Recommandations : 1140x320px, 160ko maximum -
                  {' '}
                  {(this.state.newCover) ? `Sélectionné : ${this.state.newCover}` : 'Aucun fichier sélectionné'}
                </label>
              </div>
              <div className="form-group">
                <label>
Logo de votre entreprise
                  {(this.state.logo) ? <a href={`${process.env.REACT_APP_CONTENT_DOMAIN}/${this.state.logo}`} target="_blank" rel="noopener noreferrer">Visualiser le logo actuel</a> : null}
                </label>
...
className="form-control"
id="logo"
onChange={() => { this.setState({ newLogo: document.getElementById('logo').files[0].name }); }}
style={{
                  position: 'absolute', height: '5.5em', top: '0', left: '0', opacity: '0.0001',
                }}
                  />
							Glissez votre
                  {' '}
                  {(this.state.logo) ? 'nouveau' : null}
                  {' '}
logo ici ou cliquez pour en sélectionner un parmi vos fichiers
                  <br />
							Recommandations : 200x200px, 30ko maximum -
                  {' '}
                  {(this.state.newLogo) ? `Sélectionné : ${this.state.newLogo}` : 'Aucun fichier sélectionné'}
                </label>
...
Présentation générale de l’entreprise (
                  {600 - this.state.description.length}
                  {' '}
caractères restants)
                </label>
...
Notre engagement en faveur de la biodiversité (
                  {800 - this.state.involvement.length}
                  {' '}
caractères restants)
                </label>
              ...
              <Input type="text" name="link1_url" value={this.state.link1_url} placeholder="URL du bouton d'action 1" onChange={handleChange.bind(this)} />
              <br />
              ...
              <div className="form-group">
                <label htmlFor="english">
                  <Input disabled={!(this.state.bundle_state >= 2)} type="checkbox" name="english" id="english" onChange={handleTick.bind(this)} checked={this.state.english} />
                  {' '}
Version anglaise
                </label>
              </div>
              <div className="form-group">
                <label htmlFor="visible">
                  <Input disabled={!(this.state.bundle_state >= 2)} type="checkbox" name="visible" id="visible" onChange={handleTick.bind(this)} checked={this.state.visible} />
                  {' '}
Rendre ma page publique
                </label>
                {' '}
                {(this.state.bundle_state < 2) ? <span>(Fonctionnalité indisponible tant que le paiement du parrainage n’est pas validé)</span> : null}
              </div>
